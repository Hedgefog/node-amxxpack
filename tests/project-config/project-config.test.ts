...
import { IProjectConfig, IResolvedProjectConfig } from '../../src/common/types';
...
describe('Project Config Resolver', () => {
  ...
  for (const key of ['scripts', 'include', 'assets'] as (keyof IResolvedProjectConfig['targets'])[]) {
    describe(`input ${key}`, () => {
      it('should resolve input with defaults', async () => {
        ...
        const projectConfig = createProjectConfig(config.project.defaultType, overrides, projectDir);
    ...
    it('should resolve default flat value', async () => {
      const projectConfig = createProjectConfig(config.project.defaultType, {}, projectDir);
  ...
  it('should resolve undefined paths as defaults', async () => {
    const defaultConfig = createProjectConfig(config.project.defaultType, {}, projectDir);

    const projectConfig = createProjectConfig(config.project.defaultType, {
      input: {
        scripts: undefined,
        include: undefined,
        assets: undefined
